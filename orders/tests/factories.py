...
def create_order_with_items(customer=None, products_and_quantities=None, **order_kwargs):
    """
...
def create_order_workflow_scenario():
    ...
    order = OrderFactory(
        customer=customer,
        delivery_address='123 Workflow Test Street'
    )
...
def create_bulk_orders(customer=None, count=10):
    ...
    for i in range(count):
        order = OrderFactory(
            customer=customer,
            delivery_address=f'Bulk Test Address {i}'
        )
